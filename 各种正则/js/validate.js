...
{
	var tel_reg = /^1[3|4|5|6|7|8|9]\d{9,9}$/;
...
{
	var reg_IDcard = /(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
...
function validateLoginName(user_name){
...
function validatePayword(pwd)
{
	var payword_reg = /^\d{6,6}$/;
	if (!(payword_reg.test(pwd))){
		return false;
	}
	else{
		return true;
